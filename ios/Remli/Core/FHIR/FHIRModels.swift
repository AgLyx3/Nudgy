...
/// Minimal FHIR R4 decoding, scoped to the resources Remli actually reads.
///
...
enum FHIRResource: Hashable {
    ...
    /// A resource type Remli does not read. Kept so import counts stay honest.
...
struct FHIRBundle: Decodable {
    ...
    /// Honors the `data-origin` tag Remli stamps on authored sample bundles.
    var declaredOrigin: DataOrigin? {
        guard let tag = meta?.tag?.first(where: {
            $0.system == "https://remli.app/fhir/CodeSystem/data-origin"
        }) else { return nil }
