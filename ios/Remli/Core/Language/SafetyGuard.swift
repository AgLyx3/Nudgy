...
enum SafetyRule: String, Codable, Hashable, CaseIterable {
    ...
    var rationale: String {
        ...
        case .unattributedAdministrationInstruction:
            return "Restating a dosing instruction is fine when it is quoted from the record. Asserting one as Remli's own is advice."
        case .ungroundedClockTime:
        ...
        case .diagnosisOrTriage:
            return "Diagnosis and urgent triage are explicitly excluded from v1. Remli must not be the thing that decides whether something is an emergency."
        case .emptyOutput:
...
struct SafetyVerdict: Hashable {
    ...
    /// Remli's own rule table, and for grounding rules it is by definition a token that does *not*
...
/// ## The three rules
///
/// 1. **Imperative clinical advice** — Remli may restate, attribute, and hedge. It may not
...
enum SafetyGuard {
    ...
    static func review(
        ...
        // characters of nothing. The shortest reply Remli would ever legitimately make ("Okay,
    ...
    ///
    /// So Remli makes the innocuous numbers **grounded rather than exempt**.
    ...
    private static func checkGrounding(
        ...
        //
        // Remli is expected to choose reminder times, so "I've set this for 9:30 — your other two
        ...
        for sentence in sentences {
            ...
            if isRemliOffer(phrase) || isNegatedRecordStatement(phrase) { continue }
            let inSentence = tokens(in: sentence)
            for time in inSentence.clockTimes.sorted() where !allowed.clockTimes.contains(time) {
                return .rejected(.ungroundedClockTime, evidence: time)
            }
        ...
        for number in produced.numbers.sorted() where !allowed.numbers.contains(number) {
            // A number bound to a duration unit, in a sentence where Remli is describing its own
            // schedule, is spacing arithmetic rather than a clinical quantity: "I've put these
            // twelve hours apart" is Remli explaining itself. Dose units are untouched, so
        ...
        for pair in produced.quantities.sorted() where !allowed.quantities.contains(pair) {
            // Same carve-out as numbers: a quantity whose unit is a duration is Remli describing
        ...
        for sentence in sentences {
            ...
            // An anchor offered as Remli's own idea is permitted; an anchor asserted as something
            // the chart said is not. See `isRemliOffer` for why this distinction is the whole rule.
            if isRemliOffer(phrase), !attributesToRecord(phrase) { continue }
    ...
    /// Remli. Either an explicit attribution phrase, or a quoted span that really is in the corpus.
    ...
    /// True when the sentence offers something as Remli's own idea rather than asserting it as a
    ...
    ///
    /// Both name a time anchor the chart never stated — which is exactly the point. Remli is
    ...
    /// ("9:30"), both of which Remli is meant to propose. It deliberately does not relax
    ...
    /// inside sentences where Remli is describing the schedule it chose.
    ///
    /// Deliberately narrow. It matches the number *immediately followed by a time unit*, and only
    /// in a sentence that already reads as Remli's own statement. A dose never satisfies both.
    static func isSchedulingDuration(_ number: String, in sentences: [String]) -> Bool {
        let units = Self.durationUnits
        for sentence in sentences {
            let phrase = phraseForm(of: sentence)
            guard isRemliOffer(phrase), !attributesToRecord(phrase) else { continue }
    ...
    ///
    /// This outranks `isRemliOffer`, because a sentence can wear both hats: "I'll remind you at
    ...
    static func isRemliOffer(_ phrase: String) -> Bool {
        let offers = [
            ...
            // Remli stating what it has done or will do with the schedule it owns.
...
extension SafetyGuard {
    ...
    /// Administration instructions. Fine when quoted from the record, forbidden as Remli's own
    ...
    /// Phrases that mark a sentence as reporting the record rather than asserting Remli's opinion.
