...
enum LiteRTRuntime {
    ...
    /// engine (google-ai-edge/LiteRT-LM issue #2504). Fighting that would waste the demo; Remli
    ...
    /// generous headroom over the longest prompt Remli builds.
    static let maxNumTokens = 4096

    /// Low on purpose. Remli is not looking for creative phrasing — every extra degree of
...
///
/// A single LiteRT-LM engine is not something to drive from two places at once, and Remli has an
...
final class LiteRTGemmaModel: RemliLanguageModel, LiteRTUnloadable {
