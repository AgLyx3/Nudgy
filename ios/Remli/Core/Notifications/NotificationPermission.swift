...
/// Tracks whether iOS will actually deliver Remli's reminders, and explains the answer in plain
/// language.
///
...
final class NotificationPermission: ObservableObject {
    ...
    /// Whether Remli has asked during this install, so the UI can offer the prompt at a calm moment
    ...
    /// medication reminder that does not work. Remli would rather be told no.
    @discardableResult
    ...
    var explanation: String {
        ...
        case .notDetermined:
            return """
            Remli hasn't asked to send you notifications yet. Approved reminders are delivered by \
            your phone alone — nothing is sent through a server, and no text message leaves this \
            device.
            """
        case .denied:
            return """
            Notifications are off for Remli, so approved reminders won't reach you. iOS only asks \
            once, so this has to be changed in Settings: open Settings › Notifications › Remli and \
        ...
        case .authorized:
            if alertsEnabled {
                ...
                private on the lock screen, Remli never puts a medication name or a clinic name in \
        ...
        case .provisional:
            ...
            Settings › Notifications › Remli you can choose "Deliver Prominently".
            """
        case .ephemeral:
            return """
            Notifications are allowed for this temporary session only. Install Remli to keep \
            reminders running.
            """
        @unknown default:
            return """
            This version of iOS reports a notification setting Remli doesn't recognise. You can \
            check it in Settings › Notifications › Remli.
            """
        }
    }
    ...
    /// Deep link to Remli's own page in Settings.
    func openSettings() {
