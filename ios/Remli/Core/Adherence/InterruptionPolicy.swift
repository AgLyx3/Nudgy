...
/// What Remli remembers about its own interruptions.
///
/// Read what is *not* in here: no answer text, no option identifier, no note about what the user
/// said. This file records when Remli spoke and whether anyone replied — facts about the app's
...
struct InterruptionHistory: Codable, Hashable, Sendable {
    ...
    struct TopicState: Codable, Hashable, Sendable {
        ...
        /// How many times Remli has raised this topic, ever.
        ...
        /// Set only for answers that are Remli's business. See `record(answered:)`.
    ...
    /// The last time Remli interrupted about anything at all. Backs the global budget.
    ...
    /// Every topic the user has closed for good. Surfaced in Settings so "Remli went quiet about
...
/// Why Remli said nothing. Useful in the debug console, and useful when someone asks "why didn't it
...
/// Remli is entitled to, which is: almost none.
///
/// # The product rule
///
/// > The name is Remli and the joke is she hardly ever nudges.
///
...
struct InterruptionPolicy: Sendable {
    ...
    /// Remli asked. Starts the 14-day topic cooldown and spends the global budget.
    ///
    ...
    ///
    /// The ignored streak resets, and — for answers that are Remli's own business — a timestamp
    ...
    /// instruction about Remli's behaviour and it deserves to be honoured literally.
    ///
    ...
    /// `record(raised:)` is already enough to keep Remli from asking again.
    ///
...
// compiled. `RemliSession` owns that persistence directly against `EncryptedVault`, so the
