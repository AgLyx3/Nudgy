...
/// A clock time Remli is offering, which the record did not name.
///
...
struct ProposedSlot: Codable, Hashable, Identifiable {
    ...
    var label: String
    /// Remli's offered clock time, when the record named none. Never auto-applied.
...
/// Something Remli noticed but will not resolve on its own.
...
struct ReminderProposal: Codable, Hashable, Identifiable {
    ...
    var activationTier: ActivationTier {
        ...
        // frequency is a dosing question Remli will not answer on its own.
...
struct ApprovedReminder: Codable, Hashable, Identifiable {
    ...
    func notificationIdentifiers() -> [String] {
        times.indices.map { "remli.reminder.\(id).\($0)" }
    }
}

/// Recorded when the user skips a proposal, so Remli does not re-propose it every import.
