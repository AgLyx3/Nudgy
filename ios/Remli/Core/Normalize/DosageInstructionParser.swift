...
/// Turns a FHIR `Dosage` into the fields Remli is willing to state, each with a citation.
///
...
///   Remli does not apply it, because "the spec says assume once" is still an assumption about a
///   medication.
...
struct DosageInstructionParser {
    ...
    struct ParsedDosage: Hashable {
        ...
        /// Set when `timing.repeat` carried a `periodUnit` Remli does not recognize. The schedule
        /// is nil in that case; this field lets the UI say so instead of staying silent.
        var unrecognizedPeriodUnit: String?

        /// True when the record supplied nothing Remli can turn into a cadence.
    ...
    func parseSchedule(
        ...
        // Both must be stated. Remli does not apply FHIR's implicit `frequency` default of 1.
    ...
    /// that expanding a "how often" chip shows the structured field rather than Remli's prose.
