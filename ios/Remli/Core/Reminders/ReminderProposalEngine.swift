...
struct ReminderProposalEngine {
    ...
    static func proposalID(forMedication item: MedicationItem) -> String {
        "remli.proposal.medication.\(item.id)"
    }

    static func proposalID(forTherapy task: TherapyTask) -> String {
        "remli.proposal.therapy.\(task.id)"
    }
            ...
            flags.append(
                ProposalFlag(
                    ...
                    detail: "Your record lists this medication as taken only when needed, "
                        + "so Remli is not proposing a repeating time for it.",
                    citations: [citation],
                    suggestedAction: "You can ask Remli for a one-off reminder whenever you want one."
                )
            )
                ...
                flags.append(
                    ProposalFlag(
                        ...
                        detail: "Your chart says this is \(schedule.plainLanguage). "
                            ...
                            + "Remli has suggested clock times of its own, marked as suggestions, "
            ...
            flags.append(
                ProposalFlag(
                    ...
                    detail: "Your record lists this medication but does not say how often it is taken, "
                        + "so Remli is not proposing a schedule for it.",
    ...
    /// Everything Remli is willing to state about a medication, quoted, with citations.
    ...
    private func nutritionDrafts(for guidance: [DietaryGuidance]) -> [Draft] {
        ...
        return MealAnchor.allCases.compactMap { meal -> Draft? in
            ...
            let id = "remli.proposal.nutrition.\(meal.rawValue)"
            ...
            let slot = ProposedSlot(
                ...
                provenance: .convenienceSuggestion(
                    basis: "Remli's own default time for \(meal.title.lowercased()). "
    ...
    ///
    /// Remli states the discrepancy and shows both records verbatim with both citations. It does
    ...
    private func conflictDetail(_ a: MedicationItem, _ b: MedicationItem) -> String {
        ...
        return [
            ...
            foodNote(a),
            foodNote(b),
            "Remli does not know which of these is current, and will not choose between them."
        ].joined(separator: " ")
    }
    ...
    ///
    /// `TherapyTask` has a single citation slot, but the fields Remli quotes come from different
                ...
                flags.append(
                    ProposalFlag(
                        ...
                        detail: "Nothing in the written instruction names equipment for this exercise. "
                            + "That is not the same as saying none is needed — Remli only reports what "
                            + "the record states.",
                        citations: [task.citation],
    ...
    ///
    /// Explicitly framed as a phone decision. Remli never says anything about whether taking
    ...
    private func clusterFlags(for drafts: [Draft]) -> [String: [ProposalFlag]] {
        ...
        while start < points.count {
            ...
            if distinct.count >= options.minimumClusterSize, !consumed.contains(start) {
                ...
                let flag = ProposalFlag(
                    severity: .info,
                    title: "Remli's suggested times cluster around \(Self.clockText(minutes: anchor))",
                    detail: "\(Self.list(names)) would be reminded within "
                        + "\(options.clusterWindowMinutes) minutes of each other, at times Remli suggested "
