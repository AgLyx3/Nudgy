...
///
/// Remli still wants to be useful, so each slot also carries an optional `TimeSuggestion` — a
/// default clock time Remli offers, held in a separate field from `timeOfDay`. That separation is
/// the point: the UI can render
///
/// > Dose 1 of 2 — your record does not name a time. Remli suggests 8:00 AM.
///
...
/// quietly promote Remli's idea into a chart fact.
///
/// One slot per dose, id `<idPrefix>#slot<N>`.
struct ScheduleResolver {

    struct Options {
        /// Default clock times Remli offers when the record names none. Pure UX defaults with no
    ...
    /// A `Timing.repeat.when` code, its plain meaning, and the clock time Remli would *suggest*
    /// for it.
    ///
    /// The label is the code's defined meaning in the FHIR EventTiming value set — decoding the
    /// record's own wording, not adding to it. The suggested time is Remli's, and is only ever
    ...
    ///     honoured — a record-sourced time with no citation is not something Remli will show.
    ...
    func slots(
        ...
        for index in 0..<count {
            ...
            // --- Remli's own offered clock time, attached to the same slot. ---
            //
            // One slot per dose, carrying an optional suggestion. The record fact and Remli's idea
            ...
            if let meaning {
                ...
                basis = "Remli's own default clock time for a reminder labelled “\(meaning.label)”. "
                ...
                basis = "Remli's own default spacing for \(count) reminder\(count == 1 ? "" : "s") a day. "
            ...
            // chore. Crucially the *provenance* still says this is Remli's idea — pre-filling
    ...
    /// Remli's own clock times. Nothing here is derived from the record, the drug, or anything
