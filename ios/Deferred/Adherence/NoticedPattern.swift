...
/// The three things Nudgy is allowed to notice.
///
        ...
        case .timeFit:
            return [
                ...
                PatternAnswerOption(
                    ...
                    // No concrete target: the user picks. Nudgy does not guess a second time.
                    action: .rescheduleSlot(slotIndex: nil, to: nil),
...
/// What Nudgy will do with an answer.
///
/// # Scope boundary (rule D)
///
/// Answers about *the reminder* — wrong time, ran out, too many at once — are Nudgy's job, and
...
/// **write nothing down**.
///
/// Nudgy is a reminder app. It is not a symptom journal. It does not record side effects, does not
...
enum PatternResponseAction: Codable, Hashable, Sendable {
    ...
    /// Out of Nudgy's scope. Acknowledge, point at the care team, **store nothing.**
    ...
    var persistedEffect: PersistedEffect {
        ...
        // Answering at all closes the topic for a while; that is a note about Nudgy's own
        // behaviour, containing nothing the user said.
        case .acknowledgeOnly: return .topicMute
        // Rule D. If this ever returns anything else, Nudgy has become a symptom journal.
    ...
    var isOutsideNudgysScope: Bool {
        if case .suggestCareTeam = self { return true }
        return false
    }

    /// Nudgy's reply once the user has answered.
    ///
...
struct PatternAnswerOption: Codable, Hashable, Identifiable, Sendable {
    ...
    /// Nudgy's suggestion. A user asking for a different time has just told us our guess was wrong.
...
/// Something Nudgy noticed, ready to be raised — if the interruption budget allows, which it
/// usually does not.
///
...
struct NoticedPattern: Codable, Hashable, Identifiable, Sendable {
    ...
    let windowDays: Int
    let detectedAt: Date
    /// Plain language, feeds `Provenance.patternNoticed(basis:)`. Describes what Nudgy observed
    ...
    ///
    /// Read these out loud before changing them. Every clause is either something Nudgy did
    ...
    var question: String {
        ...
        case .silence:
            ...
            // fact about Nudgy's own records and is true whether the user tapped nothing or told
