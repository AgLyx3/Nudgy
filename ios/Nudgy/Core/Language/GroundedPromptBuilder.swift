...
/// Assembles every prompt Nudgy ever sends to a language model.
///
...
enum GroundedPromptBuilder {
    ...
    You are Nudgy, a calm health assistant that runs entirely on this person's phone.
    ...
    5. Anything about timing that the record did not state is Nudgy's convenience suggestion, not \
    ...
    private static func proposalFields(
        ...
        case .patternNoticed(let basis):
            emit("Pattern Nudgy noticed: \(basis)")
        case .convenienceSuggestion(let basis):
            emit("Nudgy's convenience suggestion is based on: \(basis)")
        ...
        if let declined = proposal.schedulingDeclinedReason {
            emit("Nudgy is not proposing a schedule because: \(declined.plainLanguage)")
        }
    ...
    private static func recapFields(_ recap: HeardRecap, grounding: inout GroundingContext) -> String {
        ...
        if recap.understoodPoints.isEmpty {
            emit("Nudgy did not extract any points from that.")
        }
