...
//  THE INVARIANT
//
//  Nudgy makes exactly one promise that everything else rests on: protected health information
...
enum EgressPolicy {
    ...
    static func denialReason(for url: URL) -> String? {
        ...
        return "Blocked: \(url.host ?? "unknown host") is not on Nudgy's allowlist."
    }

    // MARK: - The PHI contract

    /// Query keys a Nudgy-constructed URL is permitted to carry.
    ///
    /// The list is short on purpose. Model downloads are addressed by path; if a URL Nudgy builds
    ...
    /// parameters Hugging Face's CDN redirects to. Nudgy follows those URLs verbatim; it never
    /// composes them.
    ...
    static var auditSummary: String {
        ...
        return (["Nudgy may contact:"] + lines + [
            "",
...
enum EgressViolation: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .hostNotAllowed(let host):
            return "Nudgy does not connect to \(host)."
        }
    }
}
