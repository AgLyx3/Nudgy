...
struct HealthSourceDescriptor: Codable, Hashable, Identifiable {
    ...
    /// One line describing *how* Nudgy is connected, e.g. "Sample file bundled with the app".
...
enum HealthSourceError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .bundledResourceNotFound(let name, let bundleDescription):
            return "Nudgy could not find the sample health file “\(name).json” in \(bundleDescription)."
        case .fileUnreadable(let path, let underlying):
            return "Nudgy could not read the health file at \(path). (\(underlying))"
        case .decodingFailed(let source, let underlying):
            return "Nudgy could not read the health records from \(source). (\(underlying))"
        case .notImplementedInV1(let capability):
            return "\(capability) is not part of this version of Nudgy."
        case .notAuthorized(let source):
            return "Nudgy is not connected to \(source) yet."
        }
    }
}
...
/// bundle, including Nudgy's authored demo export.
...
///    credentials into the provider's own web view. Nudgy never sees the password.
...
///
/// No write scopes are requested in any version. Nudgy never writes to a chart.
